...
router.get('/', async (req, res) => {
  try {
    ...
    const [rows] = await pool.query(
      'SELECT * FROM members ORDER BY name ASC LIMIT ? OFFSET ?',
      [limit, offset]
    );
...
router.post('/', protect, authorize('admin', 'editor'), async (req, res) => {
  ...
  try {
    const [result] = await pool.query(
      'INSERT INTO members (name, logo, link) VALUES (?, ?, ?)',
      [name, logo, link]
    );
    res.status(201).json({ message: 'Member created successfully', insertId: (result as any).insertId });
  } catch (error) {
